...
def add_chat(update: Update, context: CallbackContext):
    ...
    if not is_chat:
        ...
        msg.reply_text("I can already talk!!! ")
        return ""
...
def remove_chat(update: Update, context: CallbackContext):
    ...
    is_chat = sql.is_chat(chat.id)
    if not is_chat:
        msg.reply_text("It is not even enabled, keh~ ")
        return ""
    else:
        sql.rem_chat(chat.id)
        msg.reply_text("Ok i will not talk now...." )
        message = (
            f"<b>{html.escape(chat.title)}:</b>\n"
            f"#AI_DISABLED\n"
            f"<b>Admin:</b> {mention_html(user.id, html.escape(user.first_name))}\n"
...
__help__ = f"""
Chatbot utilizes the CoffeeHouse API and allows Setsuna to talk and provides a more interactive group chat experience.
